...
from vguitar.models.archive.circe3 import _onepole_scan, _segments
...
def _solve_diff(f: torch.Tensor, y: torch.Tensor, ridge: float, warmup: int
    ...
    # fp32: the 4090 cripples fp64 to ~1/64 of fp32, so the per-batch solve was the
    # whole VarPro slowdown. fp32 + scale-relative ridge keeps it well-conditioned.
    hcols = fb.permute(1, 0, 2).reshape(feat + 1, -1)  # (F+1, B*Tw)
    yt = y[:, warmup:].reshape(-1)  # (B*Tw,)
    a = hcols @ hcols.T
    a += ridge * a.diag().mean() * torch.eye(feat + 1, device=f.device)
